...
| AD-20 | Deploy-time settings can live in `config/secrets/deploy.py`, read by both `deploy_aws.sh` and `deploy_aws.py` through one loader | the deploy scripts are not long-running, so `export`-before-run was the obvious shape, but an operator who does not want to type nine variables into a shell has no equivalent to `config/secrets/config.py` for that step. This is a second, separate file rather than reusing `config.py`: deploy-time settings (broad, one-time, deployer identity) and runtime settings (narrow, continuous, `MlFactoryBackendRole`) are different credentials for different lifetimes, and merging them would put deploy-time-only permissions on disk on the corporate server, which never needs them. `deploy_aws.sh` loads the file by shelling to `deploy_aws.py --print-shell-exports` rather than re-implementing the loader in bash, so the two entrypoints cannot drift on precedence (environment wins) or on what "left as `None`" means (falls through, exactly as if the name were absent) |
