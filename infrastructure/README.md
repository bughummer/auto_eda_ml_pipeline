...
cloudformation/  one stack: artifact bucket, KMS key, experiment table, roles, workflows
docker/          the single job image used by every SageMaker job
```

## The two workflows

`eda_state_machine.asl.json` — profiling only. It marks the experiment `EDA_RUNNING`, runs the
profiling Processing job and hands control back for feature review. The execution ends there
...
|---|---|---|---|
| `MlFactoryBackendRole` | the corporate server | start and describe executions, CRUD on the experiment table, read/write the artifact prefix, read approved datasets, invoke the allow-listed Bedrock model | create SageMaker jobs, touch any other bucket |
| `MlFactoryWorkflowRole` | `states.amazonaws.com` | create/describe/stop `mlf-*` jobs, pass the job role to SageMaker, update experiment records | read datasets or artifacts directly |
...
containers need outbound access to S3, DynamoDB and CloudWatch only; if the training subnets
...
per model, all on CPU instances, plus S3 storage for artifacts and a pay-per-request DynamoDB
table. Instance types and the parallelism cap are stack parameters, not code.
